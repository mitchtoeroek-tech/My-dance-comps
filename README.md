...
8. Family accounts create `profiles`, `children`, `results`, `enrolled_comps`, `enrolled_by_child`, and `enrolled_child_sets` with row-level security. An older `favourites` table may still exist; the app no longer reads or writes it.
9. Dancer friends create `child_friend_settings` and `child_friendships`, plus RPCs so parents can add friends by email (or invite code) and see a friend’s **enrolled** comps only (child name + comp ids — not parent email, not date of birth).
