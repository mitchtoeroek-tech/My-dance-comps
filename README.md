...
8. Then paste [`supabase/migrations/20260923_studio_role_friends.sql`](supabase/migrations/20260923_studio_role_friends.sql) and run it. This limits friends to the same approved studio and the same account role (parents with parents, dancers with dancers). It adds `parent_friendships`. Safe to re-run. Hard refresh after it succeeds. Details are in the file header.
9. Family accounts create `profiles`, `children`, `results`, `enrolled_comps`, `enrolled_by_child`, and `enrolled_child_sets` with row-level security. An older `favourites` table may still exist; the app no longer reads or writes it.
10. Dancer friends create `child_friend_settings` and `child_friendships`. After the studio-role friends file, new dancer friendships must be two dancer accounts at the same approved studio. Parents use `parent_friendships` instead. Lists show a display name or “Parent of …”, never an email.
11. Community chat creates `community_messages` with row-level security so only the two accounts on an **accepted, same-studio, same-role** friendship can read or write that thread. It also adds the table to `supabase_realtime` so new messages can appear without a full reload. Friend chats are not a public room.
12. Dancer accounts add `profiles.role` (`parent` or `dancer`), `profiles.family_id`, `profiles.linked_child_id`, `children.linked_user_id`, a `families` invite code, and email invites. Existing accounts stay parents. Child rows stay owned by the parent until a dancer login is linked. The file is safe to re-run.
13. Studio accounts widen `profiles.role` to include `studio`, add `profiles.is_admin`, a `studios` table, `children.studio_id`, and a public `studio-logos` storage bucket. Pending studios are not public and cannot be linked. The file seeds `admin_allowlist` with `mitch@greenefficientliving.com.au` and marks that existing user `is_admin` when the account already exists. It is safe to re-run.
14. Studio chat creates `studio_community_messages` for an approved studio’s open room. A parent can post when any of their dancers has that `studio_id`, a dancer can post when their linked profile has that `studio_id`, and the studio owner can post once the studio is approved. Pending and rejected studios have no chat. The file adds the table to `supabase_realtime`. It does not change `community_messages`.
15. Studio-role friends add `parent_friendships` and a trigger on both friendship tables. Inserts that cross roles or studios are rejected. Existing same-studio parent friendships keep their id so Community threads stay. Cross-studio or parent↔dancer rows are declined and those chats are removed. Email and invite-code RPCs now refuse.

If you previously ran an older family-accounts, friends, community, or dancer file, run those files again, then `20260922_studio_accounts.sql`, then `20260922_studio_community_chat.sql`, then `20260923_studio_role_friends.sql`. They are written to be safe to re-run.
...
9. **Friends at your studio:** link a dancer to an approved studio, then open **My Dancers**. Parents see other parents who have a dancer there (display name, or “Parent of Mia” — never an email). Dancers see other dancers at that studio. Add someone, and they accept. A parent cannot friend a dancer account, and a dancer cannot friend a parent. More than one studio shows a section per studio. With no studio link, the page says to link a dancer and points at **Studios**.
10. When you tap **Enrolled** for a dancer, that enrolment can be shared with accepted friends at the same studio. Un-enrol removes it from their My Comps friends list. Friends see the dancer’s name, not an email address.
11. **Community** (bottom nav) lists accepted same-studio friends. Open a thread to send and receive messages. Guests see an unlock prompt to log in. Messaging is signed-in only, and only for that accepted friendship.
...
- **Community** — **Studio chats** for families linked to an approved studio (one open room per studio; dancer name, “Parent of …”, or the studio name — never an email), plus **Friend chats** between accepted friends at the same studio (parent with parent, or dancer with dancer). The list shows their display name, not an email. Guests get a log-in / sign-up unlock. Pending and rejected studios have no chat, and strangers cannot join a studio room. `/saved` redirects home.
...
Signed-in families: dancers, enrolled comps (family-wide and per-child) and results sync to your Supabase project under row-level security. Approved studio profiles (name, logo, styles, address and contact details) are public. Pending studio sign-ups are visible to the studio and to an admin only. A dancer’s link stores the approved studio id on that dancer profile. A dancer login reads and updates only their linked profile, enrolments and results. Friends only see a dancer’s **name** (or a parent’s display name / “Parent of …”) and the comps that dancer is enrolled in — not an email address, not date of birth. Community messages are stored in Supabase and are readable only by the two accounts on that accepted same-studio friendship. Either side can remove the friend, or turn off sharing enrolled comps, at any time. Passwords and PINs are handled by Supabase Auth, not stored in this app. Username logins use an internal address at `dancers.mydancecomps.app` so they can sign in without a mailbox while Confirm email is off.
