...
- **Calendar** — month view of those filtered dates (swipe or previous/next). Dots use the same registration-status colours as the Comps chips (open / closing soon / closed / dates TBC). A star on a day means you tapped **Enrolled** for that competition. Tap a day for the comps, status, and the same Enrolled control. Favourites and enrolled comps still appear even if they sit outside the current state filter.
