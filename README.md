...
10. Dancer friends create `child_friend_settings` and `child_friendships`. Enrolled comps shared with an accepted dancer friend are still child name + comp ids — not a parent email, and not date of birth. New friends are added from the studio chat, not by email.
...
15. Studio friends create `studio_friendships`. A parent login can request other parents linked to that approved studio. A dancer login can request other dancers linked to that studio. Parents and dancers cannot friend each other. Studio owner logins are not on the list. The directory returns labels such as “Parent of Mia and Leo” or a dancer’s first name — never an email. Accepted studio friends can open a friend DM. The file is safe to re-run.

If you previously ran an older family-accounts, friends, community, or dancer file, run those files again, then `20260922_studio_accounts.sql`, then `20260922_studio_community_chat.sql`, then `20260923_studio_friendships.sql`. They are written to be safe to re-run.
